 ...
 * @author jkowalczyk
 */
public class ContractInfoFactoryImpl implements ContractInfoFactory {
    ...
    private ActionHandler contractActionHandler;
    ...
    /**
     * The unique shared instance of ContractInfoFactoryImpl
     */
    ...
    protected ContractInfo setBasicContractInfo(Contract contract, ContractInfo contractInfo) {
        ...
        contractInfo.setActionList(contractActionHandler.getActionList(contract));
    ...
    protected ContractInfo setLastActInfo(Contract contract, ContractInfo contractInfo) {
        ...
        if (!lastActRequestResult.isEmpty() && lastActRequestResult.size() ==1) {
            contractInfo.setLastActInfo(ActInfoFactoryImpl.getInstance().getActInfo(lastActRequestResult.iterator().next()));
        }
        return contractInfo;
    }
    ...
    protected ContractInfo setAuditResultTrend(Contract contract, ContractInfo contractInfo) {
        ...
        if (lastActRequestResult.size() == 2) {
            ...
            if (actTab[0] instanceof Act && actTab[1] instanceof Act) {
                int lastRawMark = ActInfoFactoryImpl.getInstance().getActInfo((Act) actTab[0]).getRawMark();
                int lastButOneRawMark = ActInfoFactoryImpl.getInstance().getActInfo((Act) actTab[1]).getRawMark();
