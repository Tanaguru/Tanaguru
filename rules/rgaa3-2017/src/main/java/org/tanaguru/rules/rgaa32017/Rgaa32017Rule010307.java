...
import static org.tanaguru.rules.keystore.RemarkMessageStore.INFORMATIVE_EMB_NOT_INDENTICAL_ARIA_LABELLEDBY_MSG;
...
public class Rgaa32017Rule010307  extends AbstractMarkerPageRuleImplementation {
    ...
    protected void select(SSPHandler sspHandler) {
    	...
    	SimpleElementSelector ses = new SimpleElementSelector("*[id]:not(embed)"); //select all element with an id except embed elements
		...
		if(!elementMarkerWithAriaLabelledby.isEmpty()) {			
			ElementChecker ec = 
					new TextNotIdenticalToAttributeChecker(
					        ...
					        new ImmutablePair(TestSolution.FAILED, INFORMATIVE_EMB_NOT_INDENTICAL_ARIA_LABELLEDBY_MSG),
							TITLE_ATTR,
    	...
    	if(!elementWithAriaLabelledby.isEmpty()) {    		
    		ElementChecker ec = 
					   new TextNotIdenticalToAttributeChecker(
							    ...
							    new TextAttributeOfElementBuilder(ID_ATTR,TEXT_ELEMENT2),
							    new TextAttributeOfElementBuilder(TITLE_ATTR,ARIA_LABELLEDBY_ATTR),
				                new ImmutablePair(TestSolution.NEED_MORE_INFO, SUSPECTED_INFORMATIVE_EMB_WITH_LABELLEDBY_DETECTED),
				                new ImmutablePair(TestSolution.NEED_MORE_INFO, CHECK_EMB_IMG_ARIA_LABELLEDBY_MSG),
				                TITLE_ATTR,
						        ARIA_LABELLEDBY_ATTR); 
