...
import static org.tanaguru.rules.keystore.CssLikeQueryStore.CANVAS_TYPE_IMG_WITH_TITLE_CSS_LIKE_QUERY;
...
import static org.tanaguru.rules.keystore.RemarkMessageStore.CHECK_CANVAS_IMG_ARIA_LABELLEDBY_MSG;
import static org.tanaguru.rules.keystore.RemarkMessageStore.CHECK_CANVAS_IMG_ARIA_MSG;
import static org.tanaguru.rules.keystore.RemarkMessageStore.CHECK_OBJ_IMG_ARIA_MSG;
import static org.tanaguru.rules.keystore.RemarkMessageStore.INFORMATIVE_CANVAS_NOT_INDENTICAL_ARIA_LABELLEDBY_MSG;
import static org.tanaguru.rules.keystore.RemarkMessageStore.INFORMATIVE_CANVAS_NOT_INDENTICAL_ATTR_MSG;
import static org.tanaguru.rules.keystore.RemarkMessageStore.INFORMATIVE_OBJ_NOT_INDENTICAL_ATTR_MSG;
import static org.tanaguru.rules.keystore.RemarkMessageStore.SUSPECTED_INFORMATIVE_CANVAS_DETECTED;
import static org.tanaguru.rules.keystore.RemarkMessageStore.SUSPECTED_INFORMATIVE_CANVAS_WITH_LABELLEDBY_DETECTED;
 ...
 *
 * For more details about the implementation, refer to <a href="http://tanaguru-rules-rgaa3.readthedocs.org/en/latest/Rule-1-3-11">the rule 1.3.11 design page.</a>
 * @see <a href="http://references.modernisation.gouv.fr/referentiel-technique-0#test-1-3-13"> 1.3.11 rule specification</a>
 */

public class Rgaa32017Rule010311 extends AbstractMarkerPageRuleImplementation {
    ...
    public Rgaa32017Rule010311 () {
        ...
        setElementSelector(new SimpleElementSelector(CANVAS_TYPE_IMG_WITH_TITLE_CSS_LIKE_QUERY));
    }
    ...
    protected void select(SSPHandler sspHandler) {
    	...
    	SimpleElementSelector ses = new SimpleElementSelector("*[id]:not(canvas)"); //select all element with an id except canvas elements
		...
		if(!elementMarkerWithAriaLabelledby.isEmpty()) {			
			ElementChecker ec = 
					new TextNotIdenticalToAttributeChecker(
					        ...
					        new ImmutablePair(TestSolution.FAILED, INFORMATIVE_CANVAS_NOT_INDENTICAL_ARIA_LABELLEDBY_MSG),
							TITLE_ATTR,
    	...
    	if(!elementWithAriaLabelledby.isEmpty()) {    		
    		ElementChecker ec = 
					   new TextNotIdenticalToAttributeChecker(
							    ...
							    new TextAttributeOfElementBuilder(ID_ATTR,TEXT_ELEMENT2),
							    new TextAttributeOfElementBuilder(TITLE_ATTR,ARIA_LABELLEDBY_ATTR),
				                new ImmutablePair(TestSolution.NEED_MORE_INFO, SUSPECTED_INFORMATIVE_CANVAS_WITH_LABELLEDBY_DETECTED),
				                new ImmutablePair(TestSolution.NEED_MORE_INFO, CHECK_CANVAS_IMG_ARIA_LABELLEDBY_MSG),
				                TITLE_ATTR,
						        ARIA_LABELLEDBY_ATTR); 
		...
		if(!getSelectionWithMarkerHandler().isEmpty()) {
    		ElementChecker ec = 
    				new TextNotIdenticalToAttributeChecker(
					           	...
					           	new ImmutablePair(TestSolution.PASSED, ""),
					           	new ImmutablePair(TestSolution.FAILED, INFORMATIVE_CANVAS_NOT_INDENTICAL_ATTR_MSG),
					           	TITLE_ATTR,
					           	ARIA_LABEL_ATTR); 
    	...
    	if(!getSelectionWithoutMarkerHandler().isEmpty()) {
			ElementChecker ec = 
					new TextNotIdenticalToAttributeChecker(
			                new TextAttributeOfElementBuilder(ARIA_LABEL_ATTR),
			                new TextAttributeOfElementBuilder(TITLE_ATTR),
			                new ImmutablePair(TestSolution.NEED_MORE_INFO, SUSPECTED_INFORMATIVE_CANVAS_DETECTED),
			                new ImmutablePair(TestSolution.NEED_MORE_INFO, CHECK_CANVAS_IMG_ARIA_MSG),
			                TITLE_ATTR,
			                ARIA_LABEL_ATTR);
