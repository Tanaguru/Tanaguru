...
public class Rgaa32017Rule110301 extends AbstractPageRuleWithSelectorAndCheckerImplementation {
    ...
    private final ElementHandler<Element> ariaLabelledbyDetected = 
            new ElementHandlerImpl();
    
    /** the input form elements  */
    private final ElementHandler<Element> ariaLabelDetected = 
            new ElementHandlerImpl();
    
    /** the input form elements  */
    private final ElementHandler<Element> labelDetected = 
            new ElementHandlerImpl();
    
    /** the input form elements  */
    private final ElementHandler<Element> titleDetected = 
            new ElementHandlerImpl();
...
//        if (ariaLabelledbyDetected.isEmpty() && 
//        		ariaLabelDetected.isEmpty() && 
//        		labelDetected.isEmpty() && 
//        		titleDetected.isEmpty()) {
//        	
//            testSolutionHandler.addTestSolution(NOT_APPLICABLE);
//            return;
//        }
//        
//        //Aria-labellebdy attribute : check if there are correspondent labels first
//    	checkTextOfLabel(ariaLabelledbyDetected);
//    	
//        //Aria-label attribute : check if there are correspondent labels with the aria-label attribute then
//    	checkInputLabel(ariaLabelDetected, ARIA_LABEL_ATTR);
//    	
//        //Label element : check if there are correspondent labels with the label element then
//    	checkTextOfLabel(labelDetected);
//    	
//        //Title attribute : check if there are correspondent labels with the title attribute then
//    	checkInputLabel(titleDetected, TITLE_ATTR);
//    	
//    	
//        if(!ariaLabelledbyDetected.isEmpty()) {
//        	
...
//                new ImmutablePair(NOT_APPLICABLE, ""),
//                TEXT_ELEMENT2);
//        	ec.check(sspHandler, ariaLabelledbyDetected, testSolutionHandler);
//        }
//        
//
//        
//        //Aria-label attribute : check if there are correspondent labels with the aria-label attribute then
//        if(!ariaLabelDetected.isEmpty()) {
//
//    		ElementChecker ec = new ElementPresenceChecker(
//                new ImmutablePair(NEED_MORE_INFO, CHECK_REPEATED_LABEL_PERTINENCE_MSG),
//                new ImmutablePair(NOT_APPLICABLE, ""),
//                ARIA_LABEL_ATTR);
//        	ec.check(sspHandler, ariaLabelDetected, testSolutionHandler);
//        }
//
//        
//        
//        //Label element : check if there are correspondent labels with the label element then
//        if(!labelDetected.isEmpty()) {
//
//    		ElementChecker ec = new ElementPresenceChecker(
//                new ImmutablePair(NEED_MORE_INFO, CHECK_REPEATED_LABEL_PERTINENCE_MSG),
//                new ImmutablePair(NOT_APPLICABLE, ""),
//                TEXT_ELEMENT2);
//        	ec.check(sspHandler, labelDetected, testSolutionHandler);
//        }
//
//        
//        
//        //Title attribute : check if there are correspondent labels with the title attribute then
//        if(!titleDetected.isEmpty()) {
//
//    		ElementChecker ec = new ElementPresenceChecker(
//                new ImmutablePair(NEED_MORE_INFO, CHECK_REPEATED_LABEL_PERTINENCE_MSG),
//                new ImmutablePair(NOT_APPLICABLE, ""),
//                TITLE_ATTR);
//        	ec.check(sspHandler, titleDetected, testSolutionHandler);
//        }
...
//							if(StringUtils.equalsIgnoreCase(el.text(),lab.get(0).text())){
	...
	/**
     * Check if there are many identical labels in the page
     * Mostly on the attributes of input elements
     * If it is, they are conserved 
     * If there are single labels, they are considered as out of scope
     * @param sspHandler
     */
	private void checkInputLabel(ElementHandler<Element> elementHandler, String attribute) {
		
		ArrayList<ArrayList<Element>> arrayListOfList = new ArrayList<ArrayList<Element>>();
        boolean added = false;
        
        
        for(Element el : elementHandler.get()) {
        	
        	//Check if the element is an input element AND has the attribute
        	
        	if(el.tagName().equals(INPUT_ELEMENT) && el.hasAttr(attribute)) {
        		
   			
	        	//Initialized the first arraylist with the first element of the handler
	    		if(arrayListOfList.isEmpty()) {
	    			
	    			arrayListOfList.add(new ArrayList<Element>());
	    			arrayListOfList.get(0).add(el);
	    			
	    		}else {
	    			
	    			//check if for the current element, the attribute is equal to an other
	    			//If it is, there are in the same list
	    			//If it is not, an other list will be create
		    		for(int i = 0; i < arrayListOfList.size(); i++) {
		    			added = false;
		    			
		    			//If it is the same label of the current arrayList content
				    	if(StringUtils.equalsIgnoreCase(el.attr(attribute),arrayListOfList.get(i).get(0).attr(attribute)) ||
				    			StringUtils.contains(el.attr(attribute),arrayListOfList.get(i).get(0).attr(attribute))) {
				    		
				    		//added to it
				    		arrayListOfList.get(i).add(el);
				    		added = true;
				    		break;	
				    	}
		    		}
		    		...
		    		//Not found 
		    		if(!added) {
		    			//Create a new arraylist for the new content which is alone
		    			arrayListOfList.add(new ArrayList<Element>());
		    			arrayListOfList.get(arrayListOfList.size()-1).add(el);
		    		}
	    		}
        	}
        }
        ...
        //Remove all lists which have a single element
        for(int i = 0; i < arrayListOfList.size(); i++) {
        	if(arrayListOfList.get(i).size() == 1) {
        		arrayListOfListToDelete.add(arrayListOfList.get(i));
        	}
        }
        arrayListOfList.removeAll(arrayListOfListToDelete);
        
        

    	//clean the handler before to add the new elements list
        elementHandler.clean();        
        for(ArrayList<Element> array : arrayListOfList) {
        	elementHandler.addAll(array);
        	textOfLabels.addAll(array);
        }
	...
	/**
     * Check if there are many identical labels in the page
     * Mostly on the text of elements
     * If it is, they are conserved 
     * If there are single labels, they are considered as out of scope
     * @param sspHandler
     */
	private void checkTextOfLabel(ElementHandler<Element> elementHandler) {
        ...
        for(Element el : elementHandler.get()) {
        	...
        	//Check if the element has a not empty text
        	if(!el.text().isEmpty()) {
	    		...
	    		if(arrayListOfList.isEmpty()) {
		    		...
		    		for(int i = 0; i < arrayListOfList.size(); i++) {
		    			...
		    			//If it is the same label of the current arrayList content
				    	if(StringUtils.equalsIgnoreCase(el.text(),arrayListOfList.get(i).get(0).text()) ||
				    			StringUtils.contains(el.text(),arrayListOfList.get(i).get(0).text())) {
